...
class Remove_Add_To_Cart_Button_Woocommerce_Admin {
    ...
    public function ratcw_remove_add_to_cart_button_product_data_fields() {
		...
		<div id='ratcw_remove_add_to_cart_button_product_data' class='panel woocommerce_options_panel'>
			<div class = 'options_group' > 
				<?php 
        wp_nonce_field( 'ratcw_save_product_meta', 'ratcw_product_meta_nonce' );
        ?>
			<?php 
        $options_array = array(
        ...
        if ( ratcbw_fs()->is_not_paying() ) {
            ...
            echo '<li><span class="dashicons dashicons-yes"></span> ' . esc_html__( 'Have an options to show/hide custom messages from shop page, category pages and product details page.', 'remove-add-to-cart-button-woocommerce' ) . '</li>';
            echo '<li><span class="dashicons dashicons-yes"></span> ' . esc_html__( 'Have options to change the default Out of Stock and Backorder messages.', 'remove-add-to-cart-button-woocommerce' ) . '</li>';
    ...
    public function ratcw_save_fields( $id, $post ) {
        if ( !isset( $_POST['ratcw_product_meta_nonce'] ) || !wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['ratcw_product_meta_nonce'] ) ), 'ratcw_save_product_meta' ) ) {
            return;
        }
        $ratcw_remove_cart_button_for = ( isset( $_POST['ratcw_remove_cart_button_for'] ) ? sanitize_text_field( wp_unslash( $_POST['ratcw_remove_cart_button_for'] ) ) : '' );
        update_post_meta( $id, 'ratcw_remove_cart_button_for', $ratcw_remove_cart_button_for );
        $ratcw_hide_price = ( isset( $_POST['ratcw_hide_price'] ) ? sanitize_text_field( wp_unslash( $_POST['ratcw_hide_price'] ) ) : '' );
        update_post_meta( $id, 'ratcw_hide_price', $ratcw_hide_price );
        $ratcw_show_login_btn_when_cart_button_hidden = ( isset( $_POST['ratcw_show_login_btn_when_cart_button_hidden'] ) ? sanitize_text_field( wp_unslash( $_POST['ratcw_show_login_btn_when_cart_button_hidden'] ) ) : '' );
        update_post_meta( $id, 'ratcw_show_login_btn_when_cart_button_hidden', $ratcw_show_login_btn_when_cart_button_hidden );
        $ratcw_msg_instead_cart_button = ( isset( $_POST['ratcw_msg_instead_cart_button'] ) ? wp_kses_post( wp_unslash( $_POST['ratcw_msg_instead_cart_button'] ) ) : '' );
        update_post_meta( $id, 'ratcw_msg_instead_cart_button', $ratcw_msg_instead_cart_button );
        $ratcw_msg_instead_product_price = ( isset( $_POST['ratcw_msg_instead_product_price'] ) ? wp_kses_post( wp_unslash( $_POST['ratcw_msg_instead_product_price'] ) ) : '' );
    ...
    public function ratcw_get_settings() {
        $settings = array(
            ...
            'disable_cart_page'                          => array(
                'name'     => esc_html__( 'Disable Cart Page', 'remove-add-to-cart-button-woocommerce' ),
                'type'     => 'checkbox',
                'desc'     => esc_html__( 'Prevent access to the cart page.', 'remove-add-to-cart-button-woocommerce' ),
                'desc_tip' => true,
                'id'       => 'ratcw_disable_cart_page',
            ),
            'disable_checkout_page'                      => array(
                'name'     => esc_html__( 'Disable Checkout Page', 'remove-add-to-cart-button-woocommerce' ),
                'type'     => 'checkbox',
                'desc'     => esc_html__( 'Prevent access to the checkout page.', 'remove-add-to-cart-button-woocommerce' ),
                'desc_tip' => true,
                'id'       => 'ratcw_disable_checkout_page',
            ),
            'cart_checkout_redirect_url'                 => array(
                'name'     => esc_html__( 'Cart/Checkout Page Redirect to', 'remove-add-to-cart-button-woocommerce' ),
                'type'     => 'text',
                'desc'     => esc_html__( 'Enter the Cart/Checkout Page Redirect URL here. By default, it will go to the Shop page if this field is empty.', 'remove-add-to-cart-button-woocommerce' ),
                'desc_tip' => true,
                'id'       => 'ratcw_cart_checkout_redirect_url',
            ),
            'ratcw_add_to_cart_message_text_color'       => array(
                'name'     => esc_html__( 'Remove Add to Cart Message Text Color', 'remove-add-to-cart-button-woocommerce' ),
                'desc'     => esc_html__( 'Change the message text color from here.', 'remove-add-to-cart-button-woocommerce' ),
                'id'       => 'ratcw_add_to_cart_message_text_color',
                'type'     => 'color',
                'css'      => 'width:100px;',
                'desc_tip' => true,
            ),
            'ratcw_add_to_cart_message_background_color' => array(
                'name'     => esc_html__( 'Remove Add to Cart Message Background Color', 'remove-add-to-cart-button-woocommerce' ),
                'desc'     => esc_html__( 'Change the message section background color from here.', 'remove-add-to-cart-button-woocommerce' ),
                'id'       => 'ratcw_add_to_cart_message_background_color',
                'type'     => 'color',
                'css'      => 'width:100px;',
                'desc_tip' => true,
            ),
            'ratcw_price_message_text_color'             => array(
                'name'     => esc_html__( 'Hide Product Price Message Text Color', 'remove-add-to-cart-button-woocommerce' ),
                'desc'     => esc_html__( 'Change the message text color from here.', 'remove-add-to-cart-button-woocommerce' ),
                'id'       => 'ratcw_price_message_text_color',
                'type'     => 'color',
                'css'      => 'width:100px;',
                'desc_tip' => true,
            ),
            'ratcw_price_message_background_color'       => array(
                'name'     => esc_html__( 'Hide Product Price Message Background Color', 'remove-add-to-cart-button-woocommerce' ),
                'desc'     => esc_html__( 'Change the message section background color from here.', 'remove-add-to-cart-button-woocommerce' ),
                'id'       => 'ratcw_price_message_background_color',
                'type'     => 'color',
                'css'      => 'width:100px;',
                'desc_tip' => true,
            ),
            'ratcw_product_ofs_message'                  => array(
                'name'     => esc_html__( 'Product Out of Stock Message', 'remove-add-to-cart-button-woocommerce' ),
                'desc'     => esc_html__( 'Replace the default Out of Stock message with this.', 'remove-add-to-cart-button-woocommerce' ),
                'id'       => 'ratcw_product_ofs_message',
                'type'     => 'textarea',
                'desc_tip' => true,
            ),
            'ratcw_product_obo_message'                  => array(
                'name'     => esc_html__( 'Product Backorder Message', 'remove-add-to-cart-button-woocommerce' ),
                'desc'     => esc_html__( 'Replace the default Backorder message with this.', 'remove-add-to-cart-button-woocommerce' ),
                'id'       => 'ratcw_product_obo_message',
                'type'     => 'textarea',
                'desc_tip' => true,
            ),
            'section_end'                                => array(
