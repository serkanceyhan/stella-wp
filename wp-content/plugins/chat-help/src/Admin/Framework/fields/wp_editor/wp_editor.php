 ...
 *
 */
if ( ! class_exists( 'CHAT_HELP_Field_wp_editor' ) ) {
  class CHAT_HELP_Field_wp_editor extends CHAT_HELP_Fields {
    ...
    public function render() {
      ...
      echo ( chat_help_wp_editor_api() ) ? '<div class="chat-help-wp-editor" data-editor-settings="'. esc_attr( wp_json_encode( $editor_settings ) ) .'">' : '';
      ...
      echo ( chat_help_wp_editor_api() ) ? '</div>' : '';
    ...
    public function enqueue() {
      ...
      if ( chat_help_wp_editor_api() && function_exists( 'wp_enqueue_editor' ) ) {
    ...
    public function setup_wp_editor_media_buttons() {
      ...
      echo 'var chat_help_media_buttons = '. wp_json_encode( $media_buttons ) .';';
      echo '</script>';
    ...
    public function setup_wp_editor_settings() {
      ...
      if ( chat_help_wp_editor_api() && class_exists( '_WP_Editors') ) {

        $defaults = apply_filters( 'chat_help_wp_editor', array(
          'tinymce' => array(
            'wp_skip_init' => true
          ),
        ) );

        $setup = _WP_Editors::parse_settings( 'chat_help_wp_editor', $defaults );

        _WP_Editors::editor_settings( 'chat_help_wp_editor', $setup );
