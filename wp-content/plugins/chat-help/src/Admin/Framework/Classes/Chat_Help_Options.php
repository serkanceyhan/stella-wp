...
if (! class_exists('Chat_Help_Options')) {
  class Chat_Help_Options extends Chat_Help_Abstract
  {
    ...
    public function add_admin_bar_menu($wp_admin_bar)
    {
      ...
      if (! empty($this->args['show_bar_menu']) && empty($this->args['menu_hidden'])) {
        ...
        $menu_icon = (! empty($this->args['admin_bar_menu_icon'])) ? '<span class="chat-help-ab-icon ab-icon ' . esc_attr($this->args['admin_bar_menu_icon']) . '"></span>' : '';
    ...
    public function set_options($ajax = false)
    {
      ...
      // Set variables.
      $data      = array();
      $noncekey  = 'chat_help_options_nonce' . $this->unique;
      $nonce     = (! empty($response[$noncekey])) ? $response[$noncekey] : '';
      $options   = (! empty($response[$this->unique])) ? $response[$this->unique] : array();
      $transient = (! empty($response['chat_help_transient'])) ? $response['chat_help_transient'] : array();

      if (wp_verify_nonce($nonce, 'chat_help_options_nonce')) {

        $importing  = false;
        $section_id = (! empty($transient['section'])) ? $transient['section'] : '';

        if (! $ajax && ! empty($response['chat_help_import_data'])) {

          // XSS ok.
          // No worries, This "POST" requests is sanitizing in the below foreach. see #L337 - #L341
          $import_data  = json_decode(wp_unslash(trim($response['chat_help_import_data'])), true);
          ...
          foreach ($sections['fields'] as $field) {
            if (! empty($field['id'])) {
              if (array_key_exists($field['id'], $this->errors)) {
                $err = '<span class="chat-help-label-error">!</span>';
              }
            }
          }
        }
    ...
    public function add_options_html()
    {
      ...
      $ajax_class    = ($this->args['ajax_save']) ? ' chat-help-save-ajax' : '';
      $sticky_class  = ($this->args['sticky_header']) ? ' chat-help-sticky-header' : '';
      ...
      do_action('chat_help_options_before');

      echo '<div class="Chat_Help chat-help-options' . esc_attr($theme . $class . $wrapper_class) . '" data-slug="' . esc_attr($this->args['menu_slug']) . '" data-unique="' . esc_attr($this->unique) . '">';

      echo '<div class="chat-help-container">';

      echo '<form method="post" action="' . esc_attr($form_action) . '" enctype="multipart/form-data" id="chat-help-form" autocomplete="off" novalidate="novalidate">';

      echo '<input type="hidden" class="chat-help-section-id" name="chat_help_transient[section]" value="1">';

      wp_nonce_field('chat_help_options_nonce', 'chat_help_options_nonce' . $this->unique);

      echo '<div class="chat-help-header' . esc_attr($sticky_class) . '">';
      echo '<div class="chat-help-header-inner">';
      echo '<div class="chat-help-admin-header">';
      ...
      $notice_class = (! empty($this->notice)) ? 'chat-help-form-show' : '';
      $notice_text  = (! empty($this->notice)) ? $this->notice : '';

      echo '<div class="chat-help-form-result chat-help-form-success ' . esc_attr($notice_class) . '">' . esc_html($notice_text) . '</div>';

      echo ($has_nav && $this->args['show_all_options']) ? '<div class="chat-help-expand-all" title="' . esc_html__('show all settings', 'chat-help') . '"><i class="icofont-outdent"></i></div>' : '';
      ...
      if ($has_nav) {
        ...
        echo '<div class="chat-help-nav chat-help-nav-' . esc_attr($nav_type) . ' chat-help-nav-options">';
        ...
        foreach ($this->pre_tabs as $tab) {
          ...
          $tab_icon  = (! empty($tab['icon'])) ? '<i class="chat-help-tab-icon ' . esc_attr($tab['icon']) . '"></i>' : '';

          if (! empty($tab['subs'])) {

            echo '<li class="chat-help-tab-item">';

            echo '<a href="#tab=' . esc_attr($tab_id) . '" data-tab-id="' . esc_attr($tab_id) . '" class="chat-help-arrow">' . wp_kses_post($tab_icon) . esc_html($tab['title']) . wp_kses_post($tab_error) . '</a>';
            ...
            foreach ($tab['subs'] as $sub) {
              ...
              $sub_icon  = (! empty($sub['icon'])) ? '<i class="chat-help-tab-icon ' . esc_attr($sub['icon']) . '"></i>' : '';
            ...
            echo '<li class="chat-help-tab-item"><a href="#tab=' . esc_attr($tab_id) . '" data-tab-id="' . esc_attr($tab_id) . '">' . wp_kses_post($tab_icon) . esc_html($tab['title']) . wp_kses_post($tab_error) . '</a></li>';
          }
        }
      ...
      foreach ($this->pre_sections as $section) {
        ...
        echo '<div class="chat-help-section hidden' . esc_attr($section_onload . $section_class) . '" data-section-id="' . esc_attr($section_parent . $section_slug) . '">';
        echo ($has_nav) ? '<div class="chat-help-section-title"><h3>' . wp_kses_post($section_icon) . esc_html($section_title) . '</h3></div>' : '';
        ...
        if (! empty($section['fields'])) {
          ...
          foreach ($section['fields'] as $field) {
            ...
            Chat_Help::field($field, $value, $this->unique, 'options');
          }
        } else {
      ...
      if (! empty($this->args['show_footer'])) {
        ...
        echo (! empty($this->args['footer_text'])) ? '<div class="chat-help-copyright">' . wp_kses_post($this->args['footer_text']) . '</div>' : '';
