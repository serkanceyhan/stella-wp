...
namespace Bunny_WP_Plugin\GuzzleHttp\Handler;

use Bunny_WP_Plugin\GuzzleHttp\Exception\ConnectException;
use Bunny_WP_Plugin\GuzzleHttp\Exception\RequestException;
use Bunny_WP_Plugin\GuzzleHttp\Promise as P;
use Bunny_WP_Plugin\GuzzleHttp\Promise\FulfilledPromise;
use Bunny_WP_Plugin\GuzzleHttp\Promise\PromiseInterface;
use Bunny_WP_Plugin\GuzzleHttp\Psr7\LazyOpenStream;
...
use Bunny_WP_Plugin\Psr\Http\Message\UriInterface;
/**
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function createRejection(EasyHandle $easy, array $ctx) : PromiseInterface
    {
        ...
        if ('' !== $sanitizedError) {
            $redactedUriString = \Bunny_WP_Plugin\GuzzleHttp\Psr7\Utils::redactUserInfo($uri)->__toString();
    ...
    private static function sanitizeCurlError(string $error, UriInterface $uri) : string
    {
        ...
        $redactedUriString = \Bunny_WP_Plugin\GuzzleHttp\Psr7\Utils::redactUserInfo($baseUri)->__toString();
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf) : void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \Bunny_WP_Plugin\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \Bunny_WP_Plugin\GuzzleHttp\Psr7\Utils::streamFor($sink);
