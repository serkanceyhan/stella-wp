...
if( !function_exists('ts_remove_product_hooks') ){
	function ts_remove_product_hooks( $options = array() ){
		...
		if( isset($options['show_color_swatch']) && $options['show_color_swatch'] && function_exists('stella_template_loop_product_variable_color') ){
...
if( !function_exists('ts_restore_product_hooks') ){
	function ts_restore_product_hooks(){
		if( function_exists('stella_get_theme_options') && stella_get_theme_options('ts_product_label_pos') == 'after-thumbnail' ){
		...
		remove_action('woocommerce_after_shop_loop_item', 'stella_template_loop_product_variable_color', 50);
		remove_all_filters('stella_loop_product_variable_color_number');
...
if( !function_exists('ts_countdown') ){
	function ts_countdown( $atts = array() ){
		...
		$full_text = apply_filters('themesky_countdown_full_text', true);
		?>
			...
			<div class="counter-wrapper days-<?php echo strlen($day); ?>">
				<div class="days <?php echo $day == '00' ? 'hidden' : ''; ?>" data-days="<?php echo esc_attr($day); ?>" >
					...
					<div class="ref-wrapper">
						<?php 
						if( $full_text ){
							echo esc_html( _x('days', 'Countdown Timer', 'themesky') ); 
						}
						else{
				...
				<div class="hours" data-hours="<?php echo esc_attr($hour); ?>">
					...
					<div class="ref-wrapper">
						<?php 
						if( $full_text ){
							echo esc_html( _x('hours', 'Countdown Timer', 'themesky') ); 
						}
						else{
				...
				<div class="minutes" data-minutes="<?php echo esc_attr($minute); ?>" >
					...
					<div class="ref-wrapper">
						<?php 
						if( $full_text ){
							echo esc_html( _x('mins', 'Countdown Timer', 'themesky') );
						}
						else{
				...
				<div class="seconds" data-seconds="<?php echo esc_attr($second); ?>" >
					...
					<div class="ref-wrapper">
						<?php 
						if( $full_text ){
							echo esc_html( _x('secs', 'Countdown Timer', 'themesky') );
						}
						else{
...
if( !function_exists('ts_header_social_icons') ){
	function ts_header_social_icons(){
		if( function_exists('stella_get_theme_options') && stella_get_theme_options('ts_enable_header_social_icons') ){
...
if( !function_exists('ts_use_sharethis') ){
	function ts_use_sharethis(){
		if( !function_exists('stella_get_theme_options') ){
			return false;
		}
