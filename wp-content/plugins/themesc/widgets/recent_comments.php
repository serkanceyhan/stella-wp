...
if( !class_exists('TS_Recent_Comments_Widget') ){
	class TS_Recent_Comments_Widget extends WP_Widget {
					...
					<?php foreach( (array) $comments as $comment ): $GLOBALS['comment'] = $comment; ?>
						...
						<?php endif; ?>
							<li>
								...
								<?php if( $show_comment && function_exists('stella_string_limit_words') ): ?>
								<blockquote class="comment-body"><?php echo stella_string_limit_words(wp_strip_all_tags(get_comment_text()), 15); ?></blockquote>
								<?php endif; ?>
		...
		function update( $new_instance, $old_instance ) {
			...
			$instance['show_avatar'] 		= empty($new_instance['show_avatar']) ? 0 : 1;			
