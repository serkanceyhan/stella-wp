...
if( $show_filter_area ){
    ...
    $show_filter_area_by_default = stella_show_filter_area_by_default();
}

$grid_list_default = $theme_options['ts_prod_cat_grid_list_toggle'] ? $theme_options['ts_prod_grid_list_toggle_default'] : '';

$page_column_class = stella_page_layout_columns_class($theme_options['ts_prod_cat_layout']);
...
stella_breadcrumbs_title( $show_breadcrumb, $show_page_title, woocommerce_page_title(false) );
...
stella_shop_top_product_categories();

// Koleksiyon kontrolü
if (is_tax('product_cat')) {
    $term_id = get_queried_object()->term_id;
    $is_collection = get_term_meta($term_id, 'is_collection', true);

    if ($is_collection) {
        // Koleksiyon template dosyasını yükle
        wc_get_template('archive-product-collection.php');
        get_footer();
        exit;
    }
}
    ...
    <div id="main-content" class="<?php echo esc_attr( $main_content_class ); ?>">
        ...
        <?php if ( woocommerce_product_loop() ) : ?>
            ...
            if( class_exists('WC_Widget_Layered_Nav_Filters') ){
                ...
                the_widget('WC_Widget_Layered_Nav_Filters', array('title' => esc_html__('Active filters:', 'stella')));
                echo '</div>';
            }
