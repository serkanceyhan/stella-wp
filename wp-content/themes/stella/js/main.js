jQuery(function($){
	...
	/** Sticky Menu **/
	if (typeof stella_params != 'undefined' && stella_params.sticky_header == 1) {
		ts_sticky_menu();
	}

	/** Menu Overlay **/
	if (typeof stella_params != 'undefined' && stella_params.menu_overlay == 1) {
	...
	$(document).on('click', 'a.quickshop', function(e){
		...
		$.ajax({
			type: 'POST'
			, url: stella_params.ajax_url
			, data: { action: 'stella_load_quickshop_content', product_id: product_id }
	...
	if (typeof stella_params != 'undefined' && stella_params.ajax_add_to_cart == 1 && !$('body').hasClass('woocommerce-cart')) {
	...
	/* Ajax Search */
	if (typeof stella_params != 'undefined' && stella_params.ajax_search == 1) {
		ts_ajax_search();
	}
	...
	if (!$('body').hasClass('woocommerce-cart')) {
		$(document.body).on('adding_to_cart', function(e, $button, data){
			if (wc_add_to_cart_params.cart_redirect_after_add == 'no') {
				if (typeof stella_params != 'undefined' && stella_params.add_to_cart_effect == 'show_popup' && typeof $button != 'undefined') {
					...
					$.ajax({
						type: 'POST'
						, url: stella_params.ajax_url
						, data: { action: 'stella_load_product_added_to_cart', product_id: product_id }
						, success: function(response){
		...
		$(document.body).on('added_to_cart', function(e, fragments, cart_hash, $button){
			/* Show Cart Sidebar */
			if (typeof stella_params != 'undefined' && stella_params.show_cart_after_adding == 1) {
				$('.shopping-cart-wrapper .cart-control').trigger('click');
				return;
			}
			/* Cart Fly Effect */
			if (typeof stella_params != 'undefined' && typeof $button != 'undefined') {
				if (stella_params.add_to_cart_effect == 'fly_to_cart') {
				...
				if (stella_params.add_to_cart_effect == 'show_popup') {
					var container = $('#ts-add-to-cart-popup-modal');
	...
	$(document).on('change', '.ts-tiny-cart-wrapper .qty', function(){
		...
		$.ajax({
			type: 'POST'
			, url: stella_params.ajax_url
			, data: { action: 'stella_update_cart_quantity', qty: qty, cart_item_key: cart_item_key }
			, success: function(response){
				if (!response) {
					return;
				}
	...
	$('#ts-login-form #user_login').attr('placeholder', stella_params.placeholder_form.usernamePlaceholder);
	$('#ts-login-form #user_pass').attr('placeholder', stella_params.placeholder_form.passwordPlaceholder);
...
function ts_update_tini_wishlist() {
	if (typeof stella_params == 'undefined') {
		return;
	}
	...
	jQuery.ajax({
		type: 'POST'
		, url: stella_params.ajax_url
		, data: { action: 'stella_update_tini_wishlist' }
		, success: function(response){
	...
	jQuery('.ts-header .search-content input[name="s"], #ts-search-sidebar input[name="s"]').on('keyup', function(e){
		...
		search_timeout = setTimeout(function(){
			...
			jQuery.ajax({
				type: 'POST'
				, url: stella_params.ajax_url
				, data: { action: 'stella_ajax_search', search_string: search_string, category: category }
